import {
  ...
  BadRequestException, Module, Injectable, Inject, Logger,
} from "@nestjs/common";
...
export class ImportService {
  ...
  async importFile(
    ...
    if (!rawRows.length) {
      throw new BadRequestException("File appears to be empty or has no data rows");
    }
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      const heat = this.normaliseHeat(mapping["heat"] ? String(row[mapping["heat"]] ?? "") : "");
    ...
    this.logger.log(`Import complete: ${imported} inserted, ${skipped.length} skipped`);
  ...
  private get(row: Record<string, string>, mapping: Record<string, string>, key: string): string {
  ...
  @UseInterceptors(
    FileInterceptor("file", {
      ...
      fileFilter: (_req, file, cb) => {
        if (ALLOWED_MIMES.has(file.mimetype) || file.originalname.match(/\.(csv|xlsx|xls|ods|txt)$/i)) {
          cb(null, true);
        } else {
